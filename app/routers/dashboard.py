...
# ---------------------------------------------------------------------------
# ONE-TIME: apply Bouncer CSV export results to prospects
# DELETE THIS ROUTE after running once.
# ---------------------------------------------------------------------------

@router.post("/admin/apply-bouncer-logs", response_class=HTMLResponse)
def apply_bouncer_logs(request: Request, db: Session = Depends(get_db)):
    import csv as _csv
    import glob as _glob
    from datetime import datetime, timezone
    from pathlib import Path

    def _map(status, accept_all, toxicity):
        s = status.strip().lower()
        a = accept_all.strip().lower()
        t = int(toxicity or 0)
        if s == "deliverable":
            return "catch-all" if a == "yes" or t > 5 else "valid"
        if s == "risky":
            return "catch-all"
        if s == "undeliverable":
            return "invalid"
    ...
    files = sorted(_glob.glob(str(logs_dir / "*.csv")))
    if not files:
        return HTMLResponse("<pre>No CSV files found in logs/</pre>", status_code=400)

    bouncer: dict[str, str] = {}
    for path in files:
    ...
    for prospect in db.query(Prospect).all():
        key = (prospect.email or "").strip().lower()
        if key not in bouncer:
            not_found += 1
            continue
        new_status = bouncer[key]
    ...
    dist = Counter(bouncer.values())
    return HTMLResponse(
        f"<pre>CSVs loaded: {len(files)} files, {len(bouncer)} unique emails\n"
        f"CSV breakdown: valid={dist['valid']} catch-all={dist['catch-all']} "
        f"invalid={dist['invalid']} unknown={dist['unknown']}\n\n"
        f"DB updated:  {updated}\n"
        f"DB skipped (no change): {skipped}\n"
        f"Not in CSV:  {not_found}\n\nDone. Delete this route now.</pre>"
    )
